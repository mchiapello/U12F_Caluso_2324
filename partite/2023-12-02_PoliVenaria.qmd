...
# Read data
file <- fs::dir_ls(paste0(here::here(), "/partite/2023-12-02_PoliVenaria"), 
                   regexp = "dvw$")
x <- dv_read(file)
noi <- "BCV Caluso"
...
{{< video https://youtu.be/SxETTklb81Y >}}
...
punti <- plays(x) |> 
  dplyr::filter(!is.na(skill) & !skill %in% c("Timeout", "Technical timeout") & !is.na(video_time)) |> 
  dplyr::filter(team == noi, evaluation_code == "#", !skill %in% c("Reception")) |> 
  count(player_name) |> 
  filter(n == max(n)) |> 
  mutate(player_name = paste0(player_name, " (", n, ")")) |> 
  pull(player_name) |> 
