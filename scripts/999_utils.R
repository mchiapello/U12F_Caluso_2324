...
vr_freeball <- function(x, team, by = "player", style = "ov1"){
  y <- plays(x)
  team_select <- team
  if (by == "player") {
    y %>% 
    ...
    tmp |> 
      mutate(set_number = as.character(set_number)) |> 
      bind_rows(tibble(set_number = "Total",
                       Tot = sum(tmp$Tot),
                       Err = sum(tmp$Err)))
  }
}
...
vr_score_evplot <- function(x, with_summary = FALSE, use_icons = FALSE, icons, home_colour = "darkblue", visiting_colour = "darkred", low_colour = "#800000", mid_colour = "#202020", high_colour = "#008000", font_size = 12) {
    ...
    for (tm in c("home", "visiting")) {
      for (thispid in unique(smx$pid[smx$team_hv == tm])) {
        ...
        p <- p + annotate(GeomRichText, label = txt, x = thispid - sets2 * 0.3, y = if (tm == "visiting") yr0[1] - 1.5 else yr0[2] + 1.5,
                          hjust = 0, vjust = if (tm == "visiting") "top" else "bottom", size = 2, lineheight = 1.0,
  ...
  if (use_icons && !all(is.na(sc$icon_event))) {
    sysfonts::font_add("fa6s", regular = "/Users/chiapell/personale/PALLAVOLO/U12F_Caluso_2324/scripts/fontawesome-free-6.5.1-web/webfonts/fa-solid-900.ttf")
    ex <- sc %>%
