...
#| warning: false
###################### da modificare
class <- readr::read_tsv(paste0(here::here(), "/data/classificaRaw.tsv"), show_col_types = FALSE)
players <- readr::read_csv(paste0(here::here(), "/data/elencoAtlete.csv"), show_col_types = FALSE)

if(!lubridate::dmy(params$date) %in% pres2$date){
  class <- class %>% 
        bind_rows(pres2 |> 
                    filter(date == lubridate::dmy(params$date)) |>
                    mutate(vincitori = ifelse(numero %in% params$vincitori, 1, 0)))

    fs::file_copy(paste0(here::here(), "/data/classificaRaw.tsv"),
                  paste0(here::here(), "/data/classificaRaw_old.tsv"),
                  overwrite = TRUE)
    write_tsv(class, paste0(here::here(), "/data/classificaRaw.tsv"))
