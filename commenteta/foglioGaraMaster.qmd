...
#| fig-width: 10
s1 <- px |> 
  filter(team == noi, skill == "Serve", evaluation != "Error") |> 
...
s1 <- cbind(s1, dv_xy(s1$end_zone, end = "lower"))
...
s1 |> 
  ...
  annotate(geom = "text", x = 2, y = 0, label = loro, size = 7) +
...
ace <- plays(x) |> 
  ...
  dplyr::filter(skill == "Reception", team == loro, evaluation %in% c("Error", "Negative, limited attack")) |> 
  count(player_number, player_name) |> 
  arrange(desc(n)) |> 
  slice_head(n = 3) |> 
  pull(player_number) |> 
  paste0(collapse = ", ")
